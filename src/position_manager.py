...
THETA_SHIELD_DAYS = 2
THETA_SHIELD_PRICE_RANGE_PCT = 0.5  # Если цена в коридоре ±0.5% от входа
...
class PositionManager:
    ...
    async def open_strangle(self, put_symbol, call_symbol, put_qty, call_qty, entry_premium, current_price):
        self.is_active = True
        ...
        logger.info(f"Strangle opened: put={put_symbol} qty={put_qty}, call={call_symbol} qty={call_qty}, entry_price={current_price}")

    async def update_and_check(self, current_price):
        if not self.is_active:
            return

        # --- Основная логика TP/SL ---
        ...
        # --- Theta Shield: закрываем, если цена "застыла" в боковике на 2 дня ---
        days_elapsed = (time.time() - self.entry_time) / 86400
        if days_elapsed > THETA_SHIELD_DAYS:
            price_change_pct = abs(current_price - self.entry_price) / self.entry_price
            if price_change_pct < THETA_SHIELD_PRICE_RANGE_PCT / 100:
                logger.info(f"Theta Shield triggered: price stuck in range for {days_elapsed:.1f} days")
                await self._close_position()
                return
